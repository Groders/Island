...
function M.flash(object, frames, listener)
    ...
    local function revert() object.fill.effect = nil end

    object._flashFrames = math.min(180,
                                   (frames or 30) + (object._flashFrames or 0))
...
function M.screenFlash(color, blendMode, time)
    ...
    local function destroy() display.remove(overlay) end
...
function M.bounce(object, intensity, time)
    ...
    time = time or 500
    local i = {
        y = object._y - (object.width * intensity),
        transition = easing.outBounce,
        time = time,
        iterations = -1
    }
...
function M.shake(object, frames, intensity)
    ...
    -- Add frames to count
    object._shakeFrames = math.min(180,
                                   (frames or 30) + (object._shakeFrames or 0))
    ...
    local function shake()
        if (object._shakeFrames > 0) and object.contentBounds then -- Shake it
            ...
            if object._shakeFrames % 2 == 1 then
                object._iX = (math.random(intensity) - (intensity / 2)) *
                                 (object._shakeFrames / 100)
                object._iY = (math.random(intensity) - (intensity / 2)) *
                                 (object._shakeFrames / 100)
...
function M.newTrail(object, options)
    ...
    local trans = options.transition or
                      {
            time = 250,
            alpha = 0,
            delay = 50,
    ...
    trail.ox, trail.oy, trail.oz, trail.oa = object.x, object.y,
                                             (object.z or 0), object.rotation
    trail.alpha = alpha
    ...
    local function enterFrame()
        ...
        -- Haven't moved
        if lenSqr(object.x - trail.ox, object.y - trail.oy,
                  (object.z or 0) - trail.oz) < 1 * 1 then return false end
...
function M.newStreak(options)
    ...
    for i = 1, math.floor(360 / count) do
        local streak = display.newImageRect(image, length,
                                            length / count * ratio)
        streak.anchorX = 0
    ...
    local function spin() streaks:rotate(speed) end

    function streaks:start() Runtime:addEventListener("enterFrame", spin) end

    function streaks:stop() Runtime:removeEventListener("enterFrame", spin) end

    function streaks:finalize() self:stop() end
