...
local function countGlobals()
    local count = 0
    for k, v in pairs(_G) do count = count + 1 end
    return count
end
-- =============================================================
-- Configure loader
...
ssk.init = function(params)
    ...
    params.extras = params.extras or {
        "actions.face", "actions.move", "actions.movep", "actions.scene",
        "actions.target", "android", "behaviors", "dialogs.basic",
        "dialogs.custom", "easyBench", "easyCamera", "easyInputs.joystick",
        "easyInputs.oneTouch", "easyInputs.twoTouch", "easyInputs.oneStick",
        "easyInputs.twoStick", "easyInputs.oneStickOneTouch", "easyPositioner",
        "easySocial", "factoryMgr", "files", "meters", "misc", "persist", "pex",
        "points", "ripper", "security", "shuffleBag", "soundMgr", "tiledLoader",
        "vScroller"
    }
    ...
    params.external = params.external or {
        "30log", -- "autolan",
        "global_lock", "ponyfont", "portableRandom", "proxy", "randomlua",
        "wait"
    }
    ...
    -- =============================================================
    local local_require = (params.measure) and
                              require("ssk2.measureSSK").measure_require or
                              _G.require
    ...
    --
    local toLoad = {}
    for k, v in pairs(params.extras) do toLoad[#toLoad + 1] = v end
    table.sort(toLoad)
    for i = 1, #toLoad do local_require("ssk2.extras." .. toLoad[i]) end

    --
    -- Load Externals Alphabetically
    --
    local toLoad = {}
    for k, v in pairs(params.external) do toLoad[#toLoad + 1] = v end
    table.sort(toLoad)
    for i = 1, #toLoad do local_require("ssk2.external." .. toLoad[i]) end
    ...
    Runtime:addEventListener("enterFrame", ssk)

    -- =============================================================
    -- Current Orientation Detection
    -- =============================================================
    ssk.__currentOrientation =
        (display.contentWidth < display.contentHeight) and "portrait" or
            "landscapeRight"
    ...
    -- Initialize The Core
    -- =============================================================
    if (ssk.__debugLevel > 0) then table.dump(launchArgs, "^-launchArgs\n") end
