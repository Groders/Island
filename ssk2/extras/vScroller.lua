...
function vScroller.new(group, x, y, params)
    ...
    local function passOnTouch(theScroll, event)
        ...
        if (params.listener) then params.listener(theScroll, event) end
    end
    ...
    local listener = function(event)
        ...
        elseif (event.phase == "moved") then
            local dragTrigger = (mAbs(event.y - event.yStart) >
                                    (params.dragDist or 10)) or
                                    (mAbs(event.x - event.xStart) >
                                        (params.dragDist or 10))
    ...
    function scroller.insertContent(self, obj, params)
        ...
        if (params.skipScrollBuffering) then
            ...
            if (autoScroll or params.autoScroll) then
                ...
                if (scrollBuffer) then toY = toY + scrollBuffer end
